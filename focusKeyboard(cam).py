...
def setFocus(name, ignore=[]):
    openApps = openAppList()
    if name not in openApps:
        name = generateBestMacth(name, ignore)
        if name == None:
            return False
    ...
    try:
        ...
        #print(h)
        app.connect(title = name, found_index = 0)
        app_dialog = app.window(title=name, found_index = 0)
        app_dialog.exists()
    ...
    return True
def generateBestMacth(name, ignore):
    relevantApps = openAppList()
    containsName = []
...
def startup():
    ...
    teamsApp = pywinauto.Application(backend="uia")
    zoomApp = pywinauto.application.Application()
    bank = ""
